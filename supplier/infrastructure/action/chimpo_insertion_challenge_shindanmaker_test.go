...
var _ = Describe("ChimpoInsertionChallengeShindanmaker", func() {
	...
	BeforeEach(func() {
		...
		chimpoInsertionChallengeShindanmaker = action.NewChimpoInsertionChallengeShindanmaker(c)
	})
	...
	Describe("Event()", func() {
		...
		Context("fetching fails", func() {
			BeforeEach(func() {
				c.EXPECT().Do("テスト", "https://shindanmaker.com/a/670773").Return(
					"",
			...
			It("returns an error", func() {
				_, index, err := chimpoInsertionChallengeShindanmaker.Event(service.Message{
					IsReblog: false,
		...
		Context("fetching succeeds", func() {
			...
			Context("toot does not start with name", func() {
				It("returns an event", func() {
					event, index, err := chimpoInsertionChallengeShindanmaker.Event(service.Message{
						ID:       "1",
						IsReblog: false,
						Account: service.Account{
							DisplayName: "テスト",
